...
namespace DoAn_BanHang.Controllers
{
    public class ProductController : Controller
    {
        ...
        public JsonResult JSSearch(FormCollection data)
        {
            ...
            List<tbl_ThietBi> tbs = code.Get_ThietBi().Where(m => m.MaLoaiThietBi == IDL && XoaDau(m.TenThietBi).Contains(XoaDau(search)) && m.MaTrangThaiKinhDoanh == 1 && m.SoLuong > 1).OrderBy(m => m.GiaBan).ToList();
            if (price==2)
            {
                tbs = code.Get_ThietBi().Where(m => m.MaLoaiThietBi == IDL && XoaDau(m.TenThietBi).Contains(XoaDau(search)) && m.MaTrangThaiKinhDoanh == 1 && m.SoLuong > 1).OrderByDescending(m => m.GiaBan).ToList();
            }
            ...
            foreach (tbl_ThietBi item in tbs)
            {
                ...
                {
                    SLDV++;
                    context += "<div class='col-md-4 col-sm-12'><div class='thumbnail' style='background: linear-gradient( 45deg, rgba(29, 236, 197, 0.5), rgba(91, 14, 214, 0.5) 100% );'><div class='single-products'><div class='productinfo text-center'><img src='" + item.HinhAnh + "' alt='' /><h2>" + item.GiaBan.ToString("#,##0").Replace(",", ".") + " VNĐ</h2><p>";
